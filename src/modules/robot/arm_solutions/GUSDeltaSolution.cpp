...
GUSDeltaSolution::GUSDeltaSolution(Config* config)
{
    ...
    // arm_radius is the horizontal distance from hinge to hinge when the effector is centred
    vertical_offset = config->value(vertical_offset_checksum)->by_default(79.0f)->as_number();
}

void GUSDeltaSolution::cartesian_to_actuator( const float cartesian_mm[], ActuatorCoordinates &actuator_mm )
{
...
void GUSDeltaSolution::actuator_to_cartesian( const ActuatorCoordinates &actuator_mm, float cartesian_mm[] )
{        
float xxx, yyy, zzz;
